...
const byte Servo1_Cmd_incr = byte('A'); // command to increase angle of rotation [RU: команда увеличения угла поворота]
const byte Servo1_Cmd_decr = byte('D'); // command to decrease angle of rotation [RU: команда уменьшения угла поворота]
...
// - maximum and minimum values of the angles, the angle of rotation set by default when the module is reset [RU: - максимальные и минимальные значения углов, угол поворота выставляемый по умолчанию при сбросе модуля]
const int Servo1_Deg_default = 90; // the angle of rotation by default  [RU: угол поворота по умолчанию]
const int Servo1_Deg_min = 0+5; // limiting minimum angle of rotation [RU: предельный минимальный угол поворота]
const int Servo1_Deg_max = 180-5; // limiting maximum angle of rotation [RU: предельный максимальный угол поворота]
const int Servo2_Deg_default = Servo1_Deg_default; // the angle of rotation by default  [RU: угол поворота по умолчанию]
const int Servo2_Deg_min = Servo1_Deg_min; // limiting minimum angle of rotation [RU: предельный минимальный угол поворота]
const int Servo2_Deg_max = Servo1_Deg_max; // limiting maximum angle of rotation [RU: предельный максимальный угол поворота]
// rotation speed: non-blocking delay between changes in angles of 1 degree during the rotation, if = 0 - the delay is absent [ms> = 0] [RU: - скорость поворота: неблокирующая задержка между изменениями углов на 1 градус при повороте, если =0 - задержка не вносится [мс, >=0]]
const long Servo1_Time_rotate_deg1 = 0;
const long Servo2_Time_rotate_deg1 = Servo1_Time_rotate_deg1;
// rotation speed control at reset: the blocking delay, i.e. until a predetermined angle is set, the rest of the program will not operate [ms> 0] [RU: - регулятор скорости поворота при сбросе: блокирующая задержка, т.е. пока не будет установлен заданный угол модуль не даст работать остальной части программы [мс, >0]]
const int Servo1_Time_rst = Servo1_Time_rotate_deg1;
const int Servo2_Time_rst = Servo1_Time_rotate_deg1;
// - relevance period for the rotation command (with Mode = 0): after receiving a single command rotation will be stopped after a specified period or after next command is obtained [ms> 0] [RU: - время актуальности команды на поворот (при Mode=0): после получения однократной команды поворот будет остановлен через заданное время или по получению другой команды [мс, >0]]
...
// ========== MAIN ===============
// ===============================
unsigned int CMD[1] = {0}; // interface's data, 1 byte [RU: данные от интерфейса, 1 байт]



void setup() {

  // initial configuration of available pins [RU: первичная настройка доступных пинов]
  ...
  // module's initialization [RU: инициализация модулей]
  Serial.begin(UART_Speed);
  servo1.attach(Servo1);
  servo2.attach(Servo2);

  digitalWrite(LED_tech, HIGH); // initialization's indication - begin [RU: индикация инициализации - начало]
  _Control_Servo1(1, 1, CMD[0], 0); // servos are installed to the default position [RU: установка сервоприводов в положение по умолчанию]
  _Control_Servo2(1, 1, CMD[0], 0);
  delay(1000);  
  digitalWrite(LED_tech, LOW); // initialization's indication - end [RU: индикация инициализации - конец]
...
void loop() {
  ...
  // reading the interface, the result will be available in CMD [RU: опрос интерфейса, результат будет доступен в CMD]
  _Interf_UART(0, CMD);

  // execution of control schemes [RU: выполнение схем управления]
...
static unsigned int cnt_byte;
  ...
  if (Rst == 0) {
      if (Serial.available() != 0) {
        ...
        switch (cnt_byte) { // check the integrity of the package, if there is at least one failure - reset the reception and begin the search for a new packet's header [RU: проверка целостности пакета, если хоть один сбой - сбрасываем приём и начинаем поиск заголовка нового пакета]
          case 0:
          ...
          case 4: // if the end is reached, then the packet is correct and you may send the command [RU: если дошли до конца, то пакет верен и можно выдать команду]
            cnt_byte = 0;
            Data[0] = data_uart;
            break;
...
static unsigned long time_cmd_detect, time_rotate_detect = 0; // значение времени получения команды и времени последнего изменёния угла поворота
  ...
  if (Rst == 0 & Cmd != Cmd_def) {
    ...
    if (Time_rotate_deg1 == 0) { // rotation algorithm [RU: алгоритм поворота]
        if (incr == 1) {
          if (deg < Deg_max) { deg++; } // rotate towards 180 [RU: поворачиваем к 180]
        } else if (decr == 1) { 
          if (deg > Deg_min) { deg--; } // rotate towards 0 [RU: поворачиваем к 0]
        }
        servo1.write(deg);
    } else {
      if (millis() - time_rotate_detect > Time_rotate_deg1) { // Delay while rotating - rotation speed control [RU: задержка при повороте - регулирование скорости поворота]
        if (incr == 1) {
          if (deg < Deg_max) { // rotate towards 180 [RU: поворачиваем к 180]
            deg++;
            time_rotate_detect = millis();
          }
        } else if (decr == 1) { // rotate towards 0 [RU: поворачиваем к 0]
          if (deg > Deg_min) {
            deg--;
            time_rotate_detect = millis();
          }
    ...
    // default settings [RU: установка по умолчанию]
